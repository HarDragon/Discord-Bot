...
module.exports = {
    ...
    async run(bot, message, args) {
        ...
        if (balance === null) {
            ...
            var colour = words[Math.floor(Math.random() * words.length)];


            Economy.add(`${user.id}.balance`, amount)
            Economy.subtract(`${user.id}.bank`, amount)


            const number = new MessageEmbed()
                .setColor(colour)
                .setDescription(`*Thank you for banking with Minato, ${user.username}*` + ` ***${moneysymbolfront}${amount.toLocaleString()} ${moneysymbolback}*** ` + `*has been withdrawn from your bank.*`)
